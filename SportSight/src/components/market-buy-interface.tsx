...
export function MarketBuyInterface({ marketId, market }: MarketBuyInterfaceProps) {
    ...
    try {
        ...
        const allowance = await readContract(publicClient, {
            ...
            args: [account as `0x${string}`, MarketContract.address]
        });

        const requiredAmount = parseEther(amount.toString());
        setBuyingStep(allowance < requiredAmount ? 'allowance' : 'confirm');
    } catch (err) {
        console.error(err);
    }
};
    ...
    const handleSetApproval = async () => {
        ...
        try {
            await writeContractAsync({
                ...
                args: [MarketContract.address, parseEther(amount.toString())]
            });
